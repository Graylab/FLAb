...
from embedding_help_igfold2 import create_embedding_bert, create_embedding_gt, create_embedding_structure
...
def ft_igfold2_bert_score(train_df, valid_df, test_df):
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for data_name, df_path in zip(["train", "valid", "test"], [train_file, valid_file, test_file]):
            ...
            emb = create_embedding_bert(df, emb_type="per_prot")
...
def ft_igfold2_gt_score(train_df, valid_df, test_df):
    """
    Input: DataFrames for train, valid, test
    Output: Predicted values for test
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for data_name, df_path in zip(["train", "valid", "test"], [train_file, valid_file, test_file]):
            ...
            df["heavy"] = df["heavy"].str.replace('|'.join(["O","B","U","Z","J"]), "X", regex=True)
            df["light"] = df["light"].str.replace('|'.join(["O","B","U","Z","J"]), "X", regex=True)
            emb = create_embedding_gt(df, emb_type="per_prot")
            emb.to_pickle(df_path)  # overwrite with embeddings
...
def ft_igfold2_structure_score(train_df, valid_df, test_df):
    """
    Input: DataFrames for train, valid, test
    Output: Predicted values for test
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for data_name, df_path in zip(["train", "valid", "test"], [train_file, valid_file, test_file]):
            ...
            df["heavy"] = df["heavy"].str.replace('|'.join(["O","B","U","Z","J"]), "X", regex=True)
            df["light"] = df["light"].str.replace('|'.join(["O","B","U","Z","J"]), "X", regex=True)
            emb = create_embedding_structure(df, emb_type="per_prot")
            emb.to_pickle(df_path)  # overwrite with embeddings

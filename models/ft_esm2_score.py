...
from embedding_help import create_embedding
...
def ft_esm2_score(train_df, valid_df, test_df, esm_model):
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for data_name, df_path in zip(["train", "valid", "test"], [train_file, valid_file, test_file]):
            df = pd.read_pickle(df_path)[['sequence', 'label']]
            # Replace unusual amino acids with "X"
            df["sequence"] = df["sequence"].str.replace('|'.join(["O","B","U","Z","J"]), "X", regex=True)
            emb = create_embedding(f"facebook/{esm_model}", df, emb_type="per_prot") # an example for esm_model is 'esm2_t6_8M_UR50D'
        ...
        predictions = model.predict(test_emb.iloc[:, :-2])
